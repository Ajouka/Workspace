public class SudokuSolver {
    ...
    private boolean solve(SudokuBoard board, int row, int col) {
        ...
        if (!board.isEmpty(row,col)) return solve(board, row, col + 1);
    ...
    private boolean isRight(SudokuBoard board, int row, int col, int numb) {


        for (int i = 0; i < 9; i++) {
            if (board.getCell(row,i) == numb) return false;

        }
        for (int i = 0; i < 9; i++) {
            if (board.getCell(i,col) == numb) return false;
        }
        ...
        for (int i = firstRow; i < firstRow + 3; i++) {
            for (int j = firstCol; j < firstCol + 3; j++) {
                if (board.getCell(i,j) == numb) return false;
            }
        }
        return true;
    }
